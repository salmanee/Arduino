...
package processing.app1;

import static processing.app.I18n.tr;
import static processing.app1.Theme.scale;
...
import cc.arduino1.packages.MonitorFactory;
import cc.arduino.packages.Uploader;
import cc.arduino.packages.uploaders.SerialUploader;
import cc.arduino1.view.GoToLineNumber;
import cc.arduino1.view.StubMenuListener;
import cc.arduino1.view.findreplace.FindReplace;
import jssc.SerialPortException;
import processing.app.*;
import processing.app.debug.RunnerException;
import processing.app1.forms.PasswordAuthorizationDialog;
import processing.app1.helpers.DocumentTextChangeListener;
import processing.app1.helpers.Keys;
...
import processing.app1.tools.MenuScroller;
...
public class Editor extends JFrame implements RunnerListener {
  ...
  public Editor(Base ibase, File file, int[] storedLocation, int[] defaultLocation, Platform platform) throws Exception {
    ...
    splitPane.setDividerSize(Theme.scale(splitPane.getDividerSize()));

    // the following changed from 600, 400 for netbooks
    // http://code.google.com/p/arduino/issues/detail?id=52
    splitPane.setMinimumSize(Theme.scale(new Dimension(600, 100)));
    box.add(splitPane);
    ...
    setMinimumSize(Theme.scale(new Dimension(
...
//    sketchMenu.add(item);
  ...
  private void addTools(JMenu menu, File sourceFolder) {
    ...
    File[] folders = sourceFolder.listFiles(new FileFilter() {
      ...
      public boolean accept(File folder) {
        System.out.println("checking1 " + folder);
        if (folder.isDirectory()) {
          System.out.println("checking2" + folder);
    ...
    for (File folder : folders) {
      ...
      System.out.println("checking3" + toolDirectory.toString());
      try {
        ...
        for (File archive : archives) {
          ...
          System.out.print("class name "+className + " arhc "+ archive +",");
  ...
  private void addInternalTools(JMenu menu) {
    ...
    item = createToolMenuItem("cc.arduino1.packages.formatter.AStyle");
    if (item == null) {
      throw new NullPointerException("Tool AStyle unavailable");
    }
    ...
    menu.add(createToolMenuItem("processing.app1.tools.Archiver"));
    menu.add(createToolMenuItem("processing.app1.tools.FixEncoding"));
  }
  ...
  private boolean handleSave2() {
    ...
    try {
      if (PreferencesData.getBoolean("editor.autoformat_currentfile_before_saving")) {
        Tool formatTool = getOrCreateToolInstance("cc.arduino1.packages.formatter.AStyle");
        formatTool.run();
      }
